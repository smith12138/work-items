...
url = 'http://server.fly2020mx.com:18001/admin/ai_player/edit.html'

header = {
    'Host': 'server.fly2020mx.com:18001/',
    'Cookie': 'PHPSESSID=6ivbopireua8jiqvi8hhc6gfan',
...
def change_mode(start, end):
    while True:
        try:
            ...
            data['flower'] = random.randint(0, 1)
            data['enter_high_coin'] = random.randint(1, 2)
            res = requests.post(url=url, headers=header, data=data).text
            if '非法访问' in res:
                header['Cookie'] = get_cookie(header['Host'])
                res = requests.post(url=url, headers=header, data=data).text
            if '修改成功' in res:
                print('玩家', start, '--', '更改模式', data['mode'], '成功')
            else:
                print('玩家', start, '--', '更改模式', data['mode'], '失败')
        except:
            pass
    ...
    try:
        ...
        while True:
            ...
            if '非法访问' in res:
                ...
                res = requests.post(url=url, headers=header, data=data).text
            if '修改成功' in res:
                print('玩家', start, '--', '修改昵称为:', name, '成功')
            else:
                print('玩家', start, '--', '修改昵称为:', name, '失败')
    except:
        pass
    ...
    try:
        while True:
            ...
            if '非法访问' in res:
                ...
                res = requests.post(url=url, headers=header, data=data).text
            if '修改成功' in res:
                print('玩家ID:', start, '--', info + '成功')
            else:
                print('玩家ID:', start, '--', info + '失败')
    except:
        pass
...
# change_name(449, 651, 'E:\RobotName.conf')
# change_status(600, 651)
change_mode(600, 651)
